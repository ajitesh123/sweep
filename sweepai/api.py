...
async def on_ticket_endpoint(request: Request):
    try:
        ...
        ticket_request = TicketRequest(**request_data)
        
        logger.info(
            "Processing ticket",
            extra={
                "username": ticket_request.username,
                "repo_full_name": ticket_request.repo_full_name,
                "name": f"ticket_{ticket_request.username}",
                "tracking_id": ticket_request.tracking_id,
            },
        )

        on_ticket(**ticket_request.dict())
